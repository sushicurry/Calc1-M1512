...
\allowdisplaybreaks
\begin{document}
	\title{Infinite Limits and Limits at Infinity}
	\date{ }
	\maketitle
	
	\begin{enumerate}
	\item[1.] Find $\lim_{x\to \infty} f(x)$ and $\lim_{x\to -\infty}f(x)$ for the following functions and state whether they have horizontal asymptotes or not.
	...
	\item[2.] Determine the following infinite limits.
	\begin{enumerate}
		\item $$\lim_{x\to\infty} (5 + \frac{1}{x} + \frac{10}{x^2 })$$ 
				{\em Answer}:  $5$
		\item $$\lim_{\theta \to \infty} \frac{\cos \theta}{\theta^2}$$ (Hint: similar to an example from the lecture)
				{\em Answer}:  $0$
	\end{enumerate}
