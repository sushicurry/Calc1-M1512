...
\begin{document}
	\centerline{\textbf{\Large{Math 1512 Exam 3 In-Class}}}
	...
	\begin{enumerate}
		\item[1.] (12 pts) Evaluate the following indefinite integrals
		\begin{enumerate}
			\item[a.] $$\int \sqrt{x}(2x^6 - 4 \sqrt[3]{x}) \; dx$$ 
			\vspace{8cm} 
			\item[b.] $$\int \frac{16 \cos^2 w - 81 \sin^2 w}{4 \cos w - 9 \sin w} \; dw$$ %(Hint: Factor the numerator)
		\end{enumerate}
		\newpage
		\item[2.] (18 pts) Use geometry to evaluate the following definite integral $$\int_{1}^{5} (|x - 2| + \sqrt{-x^2 + 6x - 5})\;dx.$$ (Hint: The equation under the square root is that of a circle. Complete the square to see it)
		\newpage
		\item[3.] (15 pts) Simplify the following expression - $$\frac{d}{dx} \int_{x}^{x^3} \frac{dp}{p^2 + 1}.$$ (Hint: By properties of integrals we can say that $\int_{x}^{x^3} \frac{dp}{p^2 + 1} = \int_{x}^{0} \frac{dp}{p^2 + 1} + \int_{0}^{x^3} \frac{dp}{p^2 + 1})$
		\newpage
		\item[4.] (20 pts) Evaluate the following definite integrals 
		\begin{enumerate}
			\item[a.] $$\int_{0}^{\pi/4} \sec x (\sec x + \cos x) \; dx$$ 
			\vspace{7cm} 
			\item[b.] $$\int_{-\pi/4}^{\pi/4} \sec^2 x \; dx$$ 
			\vspace{7cm}
			\item[c.] $$\int_{0}^{5} e^x \; dx$$ (Hint: What was special about $e^x$ when we took derivatives?)
		\end{enumerate}
	\end{enumerate}
